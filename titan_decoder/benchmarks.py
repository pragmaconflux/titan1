...
class TitanBenchmarks:
    ...
    def benchmark_simple_encodings(self):
        ...
        self.suite.benchmark_decoding(
            self.engine, base64.b64encode(data), "base64_encoding"
        )

        # Gzip
        self.suite.benchmark_decoding(
            self.engine, gzip.compress(data), "gzip_compression"
        )

        # Zlib
        self.suite.benchmark_decoding(
            self.engine, zlib.compress(data), "zlib_compression"
        )
        ...
        self.suite.benchmark_decoding(
            self.engine, binascii.hexlify(data), "hex_encoding"
        )
    ...
    def benchmark_data_sizes(self):
        ...
        results = self.suite.benchmark_multiple_sizes(
            self.engine, base_data, sizes, "base64_"
        )
        ...
        if len(size_list) >= 2:
            ...
            for i in range(len(size_list) - 1):
                ...
                print(
                    f"  {size1} -> {size2} bytes: {size_ratio:.1f}x size, {time_ratio:.2f}x time"
                )
