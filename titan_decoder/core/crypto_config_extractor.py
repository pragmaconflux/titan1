...
class ConfigExtractor:
    ...
    def extract_iocs(data: bytes) -> Dict[str, List[str]]:
        ...
        iocs["domains"] = re.findall(
            r"(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}", text, re.IGNORECASE
        )
        ...
        iocs["hashes"] = list(
            set(re.findall(r"\b[0-9a-f]{32}\b", text, re.IGNORECASE))
        )  # MD5
        iocs["hashes"].extend(
            re.findall(r"\b[0-9a-f]{40}\b", text, re.IGNORECASE)
        )  # SHA1
        iocs["hashes"].extend(
            re.findall(r"\b[0-9a-f]{64}\b", text, re.IGNORECASE)
        )  # SHA256
