...
class DetectionRule:
    ...
    def __init__(self, rule_id: str, name: str, description: str, severity: str, detect_fn: Callable):
...
class CorrelationRulesEngine:
    ...
    def _load_starter_rules(self):
        ...
        self.rules.append(DetectionRule(
            ...
            severity="high",
            detect_fn=lambda report, iocs: self._detect_office_macro_network(report, iocs)
        ))
        ...
        self.rules.append(DetectionRule(
            ...
            severity="high",
            detect_fn=lambda report, iocs: self._detect_multistage_infra(report, iocs)
        ))
        ...
        self.rules.append(DetectionRule(
            ...
            severity="high",
            detect_fn=lambda report, iocs: self._detect_xor_with_network(report, iocs)
        ))
    ...
    def _detect_office_macro_network(self, report: Dict[str, Any], iocs: Dict[str, Any]) -> bool:
        """Detect Office documents with macros and network IOCs."""
        nodes = report.get("nodes", [])
        has_ole = any("OLE" in node.get("method", "") for node in nodes)
        has_network = bool(iocs.get("urls") or iocs.get("ipv4_public") or iocs.get("domains"))
    ...
    def _detect_multistage_infra(self, report: Dict[str, Any], iocs: Dict[str, Any]) -> bool:
    ...
    def _detect_xor_with_network(self, report: Dict[str, Any], iocs: Dict[str, Any]) -> bool:
    ...
    def evaluate_all(self, report: Dict[str, Any], iocs: Dict[str, Any]) -> List[Dict[str, Any]]:
