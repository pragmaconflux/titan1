...
``looks_like_structured_binary`` accepts. Speculative noise is dropped rather
than reported, because a fabricated artifact costs an analyst more than a
missed one costs Titan.
...
from ...utils.helpers import looks_like_structured_binary, sha256
        ...
        for encoding, pattern, minimum, decoder in self._CANDIDATES:
            for match in pattern.finditer(window):
                ...
                if decoded == data or decoded == raw:
                    continue
                if not looks_like_structured_binary(decoded):
                    continue
