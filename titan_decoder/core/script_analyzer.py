    ...
    def analyze(data: bytes) -> Optional[ScriptAnalysis]:
        ...
        try:
            ...
            obfuscation_count = sum(
                1 for pattern in obfuscation_indicators
                if re.search(pattern, text)
            )
            analysis.obfuscation_score = min(1.0, obfuscation_count / len(obfuscation_indicators))
            ...
            # Risk assessment
            risk_factors = len(analysis.suspicious_patterns) + (analysis.obfuscation_score * 3)
            if risk_factors >= 4:
...
class BashAnalyzer:
    ...
    def analyze(data: bytes) -> Optional[ScriptAnalysis]:
        ...
        try:
            ...
            if not (text.strip().startswith('#!/bin/bash') or 
                    text.strip().startswith('#!/bin/sh') or
                    any(kw in text for kw in ['${', '$((', 'for ', 'if ['])):
                return None
            ...
            # Extract commands
            commands = re.findall(r'(?:^|\s)((?:[a-z0-9_-]+)(?:\s+\||\s+&&|\s+\|\||;|$))', text)
            analysis.commands = list(set([cmd.strip().rstrip(';|&') for cmd in commands[:15]]))

            # Risk assessment
            if len(analysis.suspicious_patterns) >= 2 or 'reverse shell' in str(analysis.suspicious_patterns):
                analysis.risk_level = 'CRITICAL'
...
class ShellcodeDetector:
    ...
    def detect_shellcode(data: bytes) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'entropy': entropy,
            'length': len(data),
            'confidence': min(1.0, len(matches) / 3.0) * (1.0 if entropy > 5.5 else 0.7)
        }
